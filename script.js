...
(function(){
  ...
  // ---------- Memes gallery ----------
  const MEMES = [
    { f: '01-meet-the-mascots', caption: 'Meet the official 2026 mascots', feature: true },
  ];
  for (let i = 2; i <= 34; i++){
  ...
  if (grid){
    ...
    MEMES.forEach((m, i) => {
      ...
      img.addEventListener('load',  () => { a.classList.remove('loading'); a.classList.add('loaded'); });
      img.addEventListener('error', () => {
        a.classList.remove('loading'); a.classList.add('failed');
        // Fallback: try JPG once
        if (!img.dataset.fallback){ img.dataset.fallback='1'; img.src = `Assets/memes/thumb/${m.f}.jpg`; a.classList.remove('failed'); }
      });
    ...
    const webpOK = (() => {
      ...
      try { return c.toDataURL('image/webp').indexOf('image/webp') === 5; } catch(e) { return false; }
    })();
    ...
    function openLB(i){
      ...
      lbIm.alt = m.caption;
      lbCp.textContent = `${String(lbIndex + 1).padStart(2, '0')} / ${MEMES.length} · ${m.caption}`;
    ...
    function next(step){ openLB(lbIndex + step); FX.tick(); FX.buzz([8]); }
    ...
    lbX.addEventListener('click', () => { closeLB(); FX.tick(); });
  ...
  if (tok){
    const crowdIo = new IntersectionObserver((entries)=>{
      entries.forEach(e=>{
        if (e.isIntersecting){
          FX.crowd(1.6);
          crowdIo.disconnect();
...
function initThemedPlayer(wrap){
  ...
  const setMuted  = (m) => { frame.classList.toggle('is-muted', m); if (mute) mute.setAttribute('aria-pressed', m?'true':'false'); };
  setPaused(true);
  setMuted(v.muted);
  ...
  v.addEventListener('play',  () => { setPlaying(true);  setPaused(false); });
  v.addEventListener('pause', () => { setPlaying(false); setPaused(true); });
  v.addEventListener('ended', () => { setPlaying(false); setPaused(true); try{ v.currentTime = 0; }catch(e){} });
  ...
  mute && mute.addEventListener('click', () => { v.muted = !v.muted; setMuted(v.muted); });
  ...
  frame.addEventListener('keydown', (e) => {
    switch (e.key) {
      ...
      case 'f': e.preventDefault(); fs && fs.click(); break;
  ...
  const io = new IntersectionObserver((entries) => {
    entries.forEach(en => { if (!en.isIntersecting && !v.paused) v.pause(); });
  }, { threshold: 0.1 });
  io.observe(frame);
}
