...
(function(global){
  ...
  let enabled = (function(){
    try { return localStorage.getItem('mascots_fx') !== 'off'; } catch(e){ return true; }
  })();
  ...
  function setEnabled(v){
    enabled = !!v;
    try { localStorage.setItem('mascots_fx', enabled ? 'on' : 'off'); } catch(e){}
    if (!ctx && enabled) init();
  ...
  function animateNumber(el, opts){
    opts = opts || {};
    const to = opts.to != null ? opts.to : parseFloat(el.dataset.to || el.textContent);
    const duration = opts.duration || 1400;
    const suffix = opts.suffix || el.dataset.suffix || '';
    const decimals = opts.decimals != null ? opts.decimals : (parseInt(el.dataset.decimals||'0',10));
    if (reducedMotion){ el.textContent = to.toLocaleString(undefined,{minimumFractionDigits:decimals,maximumFractionDigits:decimals}) + suffix; return; }
    const start = performance.now();
    function step(now){
      const p = Math.min((now - start) / duration, 1);
      const eased = 1 - Math.pow(1 - p, 3);
      const v = to * eased;
      el.textContent = v.toLocaleString(undefined,{minimumFractionDigits:decimals,maximumFractionDigits:decimals}) + suffix;
  ...
  function tilt(el, max){
    ...
    el.addEventListener('mousemove', e=>{
      ...
      const rx = (py - 0.5) * -max;
      const ry = (px - 0.5) * max;
      el.style.transform = `perspective(1100px) rotateX(${rx.toFixed(2)}deg) rotateY(${ry.toFixed(2)}deg) translateY(-6px)`;
    });
    el.addEventListener('mouseleave', ()=>{ el.style.transform = ''; });
  }
  ...
  global.FX = {
    whistle, kick, tick, crowd, horn, netRipple, goal,
    setEnabled, isEnabled,
    buzz,
    reveal, animateNumber, countOnVisible,
    magnetize, tilt, confetti, copy,
    reducedMotion
  };
})(window);
